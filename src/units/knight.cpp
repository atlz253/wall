...
Knight::Knight(int x, SDL_RendererFlip flip) : Unit::Unit()
{
    ...
    if (!textures->key("knightRun"))
        textures->loadTexture("knightRun", "res/Knight/noBKG_KnightRun_strip.png");
    _texture = textures->key("knightRun");

    if (!textures->key("knightAttack"))
        textures->loadTexture("knightAttack", "res/Knight/noBKG_KnightAttack_strip.png");
...
void Knight::process(Unit *next)
{
    ...
    else if (next->getFront() <= getFront())
    {
        if (_isRuning)
        {
            _isRuning = false;
